...
{
    public class SpaControlFragment : Fragment
    {
        ImageView _toggleSwitch, _lightToggleSwitch;
        TextView _messageText, _lightMessageText, _spaLabel, _lightLabel;
        ...
        {
            return inflater.Inflate(Resource.Layout.fragment_spa_control, container, false);
        }
        ...
        public override async void OnViewCreated(View view, Bundle savedInstanceState)
        {
            ...
            ab.Title = "Spa Control";

            var curPumpStatus = PinState.OFF;
            var curLightStatus = PinState.OFF;

            _toggleSwitch = view.FindViewById<ImageView>(Resource.Id.spa_toggle_image);
            _lightToggleSwitch = view.FindViewById<ImageView>(Resource.Id.spa_lights_toggle_image);
            _messageText = view.FindViewById<TextView>(Resource.Id.spa_message_text);
            _spaLabel = view.FindViewById<TextView>(Resource.Id.spa_pump_label);
            _lightMessageText = view.FindViewById<TextView>(Resource.Id.spa_lights_message_text);
            _lightLabel = view.FindViewById<TextView>(Resource.Id.spa_light_label);
            ...
            if (pinged)
            {
                curPumpStatus = await GetStatus(Pin.SpaPump);
                curLightStatus = await GetStatus(Pin.SpaLight);

                _toggleSwitch.Visibility = ViewStates.Visible;
                _lightToggleSwitch.Visibility = ViewStates.Visible;
                _spaLabel.Visibility = ViewStates.Visible;
                _lightLabel.Visibility = ViewStates.Visible;

                _toggleSwitch.SetOnClickListener(new OnClickListener(v =>
                {
                    Dialogs.SimpleAlert(Context, "Disabled", "The spa pump is disabled for now.").Show();
                    ...
                    //    else
                    //    {
                    //        var toggleResult = await PoolService.Toggle(Pin.SpaPump);
